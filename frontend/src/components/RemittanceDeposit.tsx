...
const RemittanceDeposit: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 space-y-6">
      {/* Top Header Banner */}
      <div className="bg-gradient-to-r from-blue-900 via-indigo-800 to-slate-900 text-white rounded-xl p-6 shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center space-x-2">
            <span className="p-2 bg-blue-500/20 rounded-lg border border-blue-400/30 text-blue-300 font-bold">₹</span>
        ...
        <div className="flex items-center gap-3 bg-white/10 px-4 py-2.5 rounded-lg border border-white/15 w-full md:w-auto justify-between">
          <label className="text-xs uppercase tracking-wider text-blue-200 font-semibold">Branch:</label>
          <select
            value={selectedBranch}
            onChange={(e) => setSelectedBranch(e.target.value)}
            className="bg-slate-900 text-white font-medium text-sm border border-slate-700 rounded px-3 py-1 outline-none focus:ring-2 focus:ring-blue-400"
          >
            {branches.map((b) => (
          ...
          <button
            ...
            className="text-blue-200 hover:text-white transition-colors text-xs bg-blue-800/60 px-2 py-1 rounded"
